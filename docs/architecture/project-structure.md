...
Service-owning `Library`, `Contracts`, `Abstractions`, `Adapter`, and `Integration` projects may reference FluentResults to publish the approved result contract. This is a deliberate narrow package exception, not a relaxation of neutral-role boundaries.
