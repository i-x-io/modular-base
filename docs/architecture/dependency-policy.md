...
- `Microsoft.CodeAnalysis.Common`, `Microsoft.CodeAnalysis.CSharp`, and `Microsoft.CodeAnalysis.Analyzers` are compiler-tool-only catalog entries. They are allowed only for `Analyzer`, `SourceGenerator`, and their focused test projects, plus the designated `ArchitectureTest` project when it performs focused C# documentation-syntax validation; they must not become runtime library dependencies.
