...
Do not add Scalar to a project until its adoption has an explicit project-level decision. The central `PackageVersion` is a version-management mechanism, not approval to consume the package. After that decision, Scalar may serve as the interactive UI over FastEndpoints-generated OpenAPI documents. It does not generate FastEndpoints metadata or enforce security; it only renders configured document endpoints/files.

## Recommended registration and use

Only after an explicit project-level adoption decision, add the centrally versioned package to the selected consuming web project:
...
For that adoption, map Scalar after mapping the OpenAPI document(s), passing the exact FastEndpoints document names. Gate both routes to development unless the decision also covers production exposure under an explicit protected authorization policy:
...
After adoption, add all intended named documents explicitly; `AddDocuments("v1", "v2")` is appropriate only when both documents are registered. Compile and browser-test this mapping in the consuming application as required by the verification checklist.

If the adoption decision also permits a non-development deployment, protect both the UI and its source document with the same documentation policy:
...
The following setup and operational guidance applies only after an explicit project-level adoption decision.
...
- After the adoption decision, follow the [FastEndpoints, JWT, OpenAPI, and Scalar recipe](../recipes/fastendpoints-jwt-openapi-scalar.md) for producer, validator, and UI ownership.
- Review [Scalar.AspNetCore supply-chain metadata](../package-guidance/supply-chain.md#scalar-aspnetcore) before adoption or upgrade.
...
- Do not treat the central pin as an adoption decision or add the `PackageReference` without an explicit project-level decision.
...
- [ ] An explicit project-level adoption decision is recorded before the project adds the `PackageReference`.
