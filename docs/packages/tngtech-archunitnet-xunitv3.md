...
Use to enforce compiled-code dependency, layering, naming, and cycle rules in the designated architecture-test project. ArchUnitNET imports compiled C# bytecode into a model and evaluates fluent rules; it does not prove runtime behavior, source-generator behavior, or deployment correctness.

## Recommended registration and use

There is no DI registration. In a project with `IXModularityProjectRole=ArchitectureTest` and `IsTestProject=true`, reference both packages directly and without versions:
...
Treat each rule as an architectural contract with a clear owner and remediation path. Keep allowed-dependency lists explicit, test an intentional violation so the rule's direction is proven, and revise exceptions in the same review as the design change. Cache the immutable architecture model to avoid repeated binary imports.
...
- Do not reference the package from production or ordinary `Test` projects.
...
- [ ] The architecture-test project has direct, versionless references to `xunit.v3` and this extension, with no direct core package reference.
